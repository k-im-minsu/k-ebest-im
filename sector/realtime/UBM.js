/**
 * @typedef {Object} UBM [업종] 실시간 시세 - (통합)업종별투자자별매매현황
...
*/
/** @typedef {(data:UBM)=>void} UBMFunc*/
/**
 * [업종] 실시간 시세 - (통합)업종별투자자별매매현황
 * @param {string} tr_key 거래소별업종코드(4) U+업종코드(3), (계좌등록/해제 일 경우 빈 문자열)
 * @param {UBMFunc} recv 서버에서 데이터 받으면 해당 함수 호출
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
exports.등록 = async(tr_key="",recv)=>{
    return await __ebestim.realadd("UBM",tr_key,recv);
}
/**
 * [업종] 실시간 시세 - (통합)업종별투자자별매매현황 실시간 등록 해제
 * @param {string} tr_key 실시간 등록시 입력했던 키값
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.해제 = async(tr_key)=>{
    return await __ebestim.realdel("UBM",tr_key);
}

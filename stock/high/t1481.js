...
/**
 * @typedef {Object} t1481 [주식] 상위종목 - 시간외등락율상위
 * @property {string} rsp_cd tr 코드
 * @property {string} rsp_msg tr 메세지
 * @property {t1481_OutBlock} t1481OutBlock
 * @property {Array<t1481_OutBlock1>} t1481OutBlock1
 */
/**
 * @typedef {Object} t1481_OutBlock [주식] 상위종목 - 시간외등락율상위 
 * @property {number} idx IDX(4)
 */
/**
 * @typedef {Object} t1481_OutBlock1 [주식] 상위종목 - 시간외등락율상위 
 ...
 */
/**
 * [주식] 상위종목 - 시간외등락율상위 (초당 1건 제한)
 * @param {string} gubun1 구분(1) - 0:전체 1:코스피 2:코스닥
 * @param {string} gubun2 상승하락(1) - 0: 상승률 1: 하락률
 * @param {string} jongchk 종목체크(1) - 0: 전체 1: 우선제외 2: 관리제외 3: 우선관리제외
 * @param {string} volume 거래량(1) - 0: 전체거래량 1: 1천주 이상 2: 5천주 이상 3: 1만주 이상 4: 5만주 이상 5: 10만주 이상 6: 50만주 이상 7: 100만주 이상
 * @param {number} idx IDX(4) - 연속조회시 OutBlock의 idx 입력
 * @param {string} tr_cont 연속 거래 여부 [Y,N]
 * @param {string} tr_cont_key 연속일 경우 그전에 내려온 연속키 값 올림
 * @param {string} mac_address 	법인인 경우 필수 세팅
 * @returns {Promise<t1481|null>}  실패시 null 반환
 */
 exports.시간외등락율상위 = async(gubun1="",gubun2="",jongchk="",volume="",idx=0,tr_cont="N",tr_cont_key="",mac_address="")=>{
    const header={

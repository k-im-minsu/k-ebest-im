/**
 * @typedef {Object} UPM [주식] 실시간 시세 - (통합)프로그램매매전체집계
...
*/
/** @typedef {(data:UPM)=>void} UPMFunc*/
/**
 * [주식] 실시간 시세 - (통합)프로그램매매전체집계
 * @param {string} tr_key 단축코드 7자리 + 공백 3자리(U+6자리코드), (계좌등록/해제 일 경우 빈 문자열)
 * @param {UPMFunc} recv 서버에서 데이터 받으면 해당 함수 호출
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
exports.등록 = async(tr_key="",recv)=>{
    return await __ebestim.realadd("UPM",tr_key,recv);
}
/**
 * [주식] 실시간 시세 - (통합)프로그램매매전체집계 실시간 등록 해제
 * @param {string} tr_key 실시간 등록시 입력했던 키값
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.해제 = async(tr_key)=>{
    return await __ebestim.realdel("UPM",tr_key);
}

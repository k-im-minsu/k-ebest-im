/**
 * @typedef {Object} KH_ [주식] 실시간 시세 - KOSDAQ프로그램매매종목별
...
*/
/** @typedef {(data:KH_)=>void} KH_Func*/
/**
 * [주식] 실시간 시세 - KOSDAQ프로그램매매종목별
 * @param {string} tr_key 단축코드 6자리 또는 8자리 (단건, 연속), (계좌등록/해제 일 경우 빈 문자열)
 * @param {KH_Func} recv 서버에서 데이터 받으면 해당 함수 호출
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.KOSDAQ프로그램매매종목별 = async(tr_key="",recv)=>{
    return await __ebestim.realadd("KH_",tr_key,recv);
}
/**
 * [주식] 실시간 시세 - KOSDAQ프로그램매매종목별 실시간 등록 해제
 * @param {string} tr_key 실시간 등록시 입력했던 키값
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.KOSDAQ프로그램매매종목별_해제 = async(tr_key)=>{
    return await __ebestim.realdel("KH_",tr_key);
}

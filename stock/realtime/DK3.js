/**
 * @typedef {Object} DK3 [주식] 실시간 시세 - KOSDAQ시간외단일가체결
...
*/
/** @typedef {(data:DK3)=>void} DK3Func*/
/**
 * [주식] 실시간 시세 - KOSDAQ시간외단일가체결
 * @param {string} tr_key 단축코드 6자리 또는 8자리 (단건, 연속), (계좌등록/해제 일 경우 빈 문자열)
 * @param {DK3Func} recv 서버에서 데이터 받으면 해당 함수 호출
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.KOSDAQ시간외단일가체결 = async(tr_key="",recv)=>{
    return await __ebestim.realadd("DK3",tr_key,recv);
}
/**
 * [주식] 실시간 시세 - KOSDAQ시간외단일가체결 실시간 등록 해제
 * @param {string} tr_key 실시간 등록시 입력했던 키값
 * @returns {Promise<boolean>}  실패, 성공 여부
 */
 exports.KOSDAQ시간외단일가체결_해제 = async(tr_key)=>{
    return await __ebestim.realdel("DK3",tr_key);
}
